...
- RevenueCat verified contract (July 16, 2026): API v2 project `proj8da4e000`, CycleBalance iOS app `appca3539a96a`, current offering `default`, exact packages `$rc_monthly` / `$rc_annual`, entitlement lookup key `CycleBalance Unlimited`, and active Apple product mappings `cyclebalance.premium.monthly` (`P1M`) / `cyclebalance.premium.annual` (`P1Y`). Secret version `v1` has proxy-service-account-only accessor IAM and no public principal. Its least-privilege key scopes are read-only `Subscriptions`, `Entitlements`, `Offerings`, and `Packages`; `Products`, `Customers`, and every other scope are `No access`. The direct attached-product endpoints used here require `Packages` / `Entitlements`, not `Products`, and the live verifier passed after redundant `Products` access was removed.
...
The helper validates the ignored local Xcode configuration without printing its values, runs the positive harness only with `DRY_RUN=true`, regenerates the project, and then reads back both configurations' effective Xcode settings. Release and `ScannerCanary` must each resolve the exact pinned proxy URL and a non-empty RevenueCat public SDK key; neither value is printed. It also confirms canonical Release keeps all six scanner settings `NO`, confirms `ScannerCanary` changes only UI and Gemini to `YES`, and confirms the shared scheme still archives with Release. Use `--no-open` to perform the same staging checks without opening Xcode. The helper has no live cloud, device, installation, archive/export, or StoreKit-credential path.

On July 16, 2026, Xcode produced an Apple Development-signed `ScannerCanary` device build for `alex.PCOS` and team `2PW989LA87`; signed entitlements read back production App Attest and HealthKit. General Kenobi was paired over the local network with Developer Mode enabled, build `18` was installed, and the `PCOS` process remained running after launch. The exact Xcode-built app was installed and activated through CoreDevice after Xcode's optional debugger-symbol transfer proved slow over Wi-Fi; it used the canary scheme's real-RevenueCat launch mode. This was only a local UI/signing/launch rehearsal: Cloud Run stayed on disabled revision `cyclebalance-meal-scan-proxy-00022-5xm` with `MEAL_SCAN_ENABLED=false` and no public IAM principal, so a photo estimate could not reach Gemini. The live seed, sandbox purchase, cancellation/pending, restore, receipt synchronization, and positive real-device TestFlight gates remain open. A real AI scan still requires the separately approval-gated audited terminal harness.
...
The live canary also pipes the pinned RevenueCat secret version directly from Secret Manager into `verify-revenuecat-offering-v2.sh`; the key is memory-only and is not placed in arguments, environment variables, files, or output. That read-only verifier pins CycleBalance iOS app `appca3539a96a` and fails closed unless the active current offering is `default`, its exact packages are `$rc_monthly` and `$rc_annual`, each package has exactly one matching active CycleBalance iOS product with `P1M` / `P1Y`, and active `CycleBalance Unlimited` has exactly those two iOS products attached. RevenueCat packages and entitlements are project-scoped, so products for another app ID are permitted but cannot satisfy any CycleBalance iOS assertion.
...
The final independent review accepted the frozen local canary/evidence boundary with no critical, important, or minor findings in the documented JSON, Unicode, control, terminal, structured-recursion, canonical-mask, identifier, and CLI-atomicity scope. Arbitrary reversible non-JSON encodings such as `\\xNN`, `%NN`, and HTML entities remain outside that claim. The live canary has not run. RevenueCat offering verification passed July 16, 2026, but preparing or even passing this development-signed canary does not close the positive sandbox-JWS real-device TestFlight gate. The exact 80-image/120-call benchmark, regenerated App Store distribution profile with production App Attest and HealthKit, signed archive/export, App Privacy/policy, localization, screenshots, and explicit TestFlight/distribution approval all remain open.
...
- [x] Verify the RevenueCat API v2 secret IAM/scopes and live current `default` / `$rc_monthly` / `$rc_annual` / CycleBalance iOS product / `CycleBalance Unlimited` mappings through memory-only readback.
- [x] Remove redundant RevenueCat `Products: Read only` while retaining `Subscriptions`, `Offerings`, `Packages`, and `Entitlements` read access, then rerun the same live verifier; completed July 16, 2026 with Secret Manager version/IAM rechecked and the key kept memory-only.
- [ ] Validate sandbox purchase, cancellation/pending, restore, and receipt synchronization on the exact candidate build without a reviewer bypass.
