...
- RevenueCat verified contract (July 16, 2026): API v2 project `proj8da4e000`, CycleBalance iOS app `appca3539a96a`, current offering `default`, exact packages `$rc_monthly` / `$rc_annual`, entitlement lookup key `CycleBalance Unlimited`, and active Apple product mappings `cyclebalance.premium.monthly` (`P1M`) / `cyclebalance.premium.annual` (`P1Y`). Secret version `v1` has proxy-service-account-only accessor IAM and no public principal. Its key scopes are read-only `Subscriptions`, `Entitlements`, `Offerings`, `Packages`, and `Products`; `Customers` and every other scope are `No access`. The direct attached-product endpoints used here require `Packages` / `Entitlements`, not `Products`; removing redundant read-only `Products` remains an owner-approved least-privilege cleanup.
...
- [ ] Remove redundant RevenueCat `Products: Read only` while retaining `Subscriptions`, `Offerings`, `Packages`, and `Entitlements` read access, then rerun the same live verifier.
