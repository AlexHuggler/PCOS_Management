...
- The previously verified production foundation is checkpointed at commit `33745c6`; exported IPA artifacts remain untracked and must never be staged.
- Preserve all unrelated work; never reset, revert, or stage files outside the task's explicit file list.
...
Add `MealScanRepeatCacheRecord` with the exact fields from the approved design: IDs, hash, optional archived feature print, Vision revision, snapshot JSON/schema, display macros, `sourceMealLoggedAt`, `createdAt`, `lastUsedAt`, `lastMatchedAt`, and reuse count.
...
- Produces: `@MainActor protocol MealScanRepeatCaching`, plus `suggestion(for:now:)`, `save(snapshot:sourceMealID:sourceMealLoggedAt:fingerprint:now:)`, `markMatched(recordID:now:)`, `markReused(recordID:now:)`, and `removeRecords(sourceMealID:)`.
...
- surfacing a suggestion updates `lastMatchedAt` but not `reuseCount`; choosing reuse updates `lastUsedAt` and increments `reuseCount`.
...
Define `MealScanRepeatCaching` with the exact methods above so `MealScanViewModel` tests can inject a spy. When saving, JSON-encode the snapshot, upsert by exact hash, persist the source meal's actual logged timestamp, set denormalized display values, and evict overflow records after sorting by `lastUsedAt` ascending.
...
- Produces: `@MainActor protocol RemoteMealScanServing` with `scan(normalizedImage:mealType:) async throws -> MealScanResult`; `GeminiRemoteMealScanService` conforms.
...
Use injected `any MealScanRepeatCaching` and `any RemoteMealScanServing` spies to prove:
...
At save time, construct the versioned snapshot from the final reviewed fields, save the meal first, then persist the repeat record with `confirmedMeal.id` and `confirmedMeal.loggedAt`. A repeat-cache save failure logs and still leaves the reviewed meal saved.
...
Add `.repeatSuggestion` to the root phase switch. Render meal name, rounded calories, protein/carbohydrate/fat summary, and a relative `sourceMealLoggedAt` labeled as `last logged`, never `last used`. Use one prominent text button for `Use Previous Meal` and one bordered/plain command for `Scan as New`. Keep card radius at the app token value, use existing botanical/lunar surfaces, allow multiline text, and provide one combined VoiceOver summary followed by the two actions.
...
- Evaluation toolkit: `docs/superpowers/plans/2026-07-11-repeat-meal-evaluation.md`
- Device and App Review rollout: `docs/superpowers/plans/2026-07-11-meal-scan-device-review-rollout.md`
...
- The 100-image/95%-precision gate is isolated in the evaluation follow-on plan and cannot silently emit an unsafe policy.
- Physical App Check validation and App Review preparation are isolated in the device/review follow-on plan.
- Public App Store submission is intentionally excluded; the follow-on plan leaves the current production scanner disabled.
