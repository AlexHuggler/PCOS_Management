...
APPLE_ROOT_CA_BASE64="${APPLE_ROOT_CA_BASE64:-}"
...
if [[ "$MEAL_SCAN_ENABLED" == "true" ]]; then
  ...
  if [[ ! "$APPLE_APP_ID" =~ ^[0-9]{5,20}$ || -z "$APPLE_ROOT_CA_BASE64" ]]; then
    echo "APPLE_APP_ID and APPLE_ROOT_CA_BASE64 are required before enabling meal scans." >&2
    exit 1
  fi
fi
...
echo "MEAL_SCAN_ENABLED=true PROJECT_ID=$PROJECT_ID REGION=$REGION $SCRIPT_DIR/deploy-cloud-run.sh"
echo
