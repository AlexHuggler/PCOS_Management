...
print_apple_iap_provisioning_handoff() {
  ...
  note "- Provision only Secret Manager resource $PINNED_APPLE_IAP_PRIVATE_KEY_SECRET_NAME from the owner's local App Store Connect .p8 file using a hidden path prompt and --data-file."
  ...
  note "- Do not access, re-prompt, rotate, or replace the existing Gemini, RevenueCat, or principal-HMAC secrets."
  note "- The principal-HMAC resource is expected at pinned version $PINNED_PRINCIPAL_HMAC_SECRET_VERSION; no secret value is ever read by this harness."
}
...
print_dry_run() {
  ...
  note "2. Verify required commands, clean branch $PINNED_BRANCH, exact repository root, and canonical project.yml Release scanner flags still NO."
  note "3. Verify private Cloud Run, MEAL_SCAN_ENABLED=false, budget mode normal, no allUsers invoker, and the pinned Firebase/Apple/RevenueCat identifiers and products."
  note "4. Verify current numeric Secret Manager versions exist and are ENABLED without accessing values."
  print_apple_iap_provisioning_handoff
  note "5. Verify $DEVICE_NAME is uniquely resolved, paired, in Developer Mode, DDI-ready, and explicitly unlocked."
  note "6. Back up existing $APP_BUNDLE_ID app data before installation under mode 0700; summary records only that backup succeeded."
  note "7. Build a nondistributable development-signed Release canary from current RC source, enabling only MEAL_SCAN_RELEASE_UI_ENABLED=YES and MEAL_SCAN_RELEASE_GEMINI_ENABLED=YES."
  note "8. Inspect bundle ID, exact proxy URL, signed UI/Gemini flags, production App Attest, Apple Development provisioning, and locked-off mock/debug-direct/fallback/similarity settings."
  ...
  note "12. Require machine-read evidence that a fresh positive request traversed App Check, Apple status/JWS, RevenueCat corroboration, quota, cache, and exactly one provider dispatch."
  note "13. Require exact local reuse to add zero server requests, zero quota/provider activity, and no quota digest change."
  note "14. Require Scan as New to add exactly one completed request, one quota decision, one fresh dispatch, and one completed provider operation; a server cache hit fails this gate."
  note "The owner must rollback between the seed and rescan windows, wait beyond the 24-hour server cache TTL with the service disabled/private, then run the rescan window under a new approval-gated invocation."
  note "15. Reject logs containing tokens, JWS values, transaction identifiers, meal names, photos/base64, bearer headers, or API keys; retain only bounded aggregate evidence mode 0600."
  note "16. On success, failure, signal, or interruption, redeploy disabled/private and read back unauthenticated 403/404 plus authenticated 503 feature_disabled."
...
assert_no_prohibited_content() {
  local file="$1"
  ! LC_ALL=C grep -Eiq \
    'Authorization:[[:space:]]*Bearer|x-firebase-appcheck|signedTransactionJWS|originalTransactionId|transactionId|meal_name|mealName|display_name|data:image|base64|AIza[[:alnum:]_-]+' \
    "$file"
}
...
verify_phase_evidence() {
  ...
  case "$phase" in
    exact-reuse)
      jq -e '
        .prohibitedContentDetected == false and
        .requestCompleted == 0 and
        .providerStarted == 0 and
        .providerCompleted == 0 and
        .quotaAllowed == 0 and
        .cacheFreshDispatch == 0 and
        .cacheServerHit == 0 and
        .appCheckRejections == 0 and
        .storeKitRejections == 0 and
        .revenueCatRejections == 0 and
        .rollingQuotaDigestBefore == .rollingQuotaDigestAfter
...
verify_source_preconditions() {
  ...
  git diff --quiet || die "Live canary requires a clean unstaged worktree"
  git diff --cached --quiet || die "Live canary requires a clean staged index"
  assert_canonical_release_flags
  PROJECT_YML_SHA_BEFORE="$(shasum -a 256 "$PROJECT_YML" | awk '{print $1}')"
  append_summary "Machine-read source: clean pinned RC branch and canonical Release scanner flags NO."
}
...
verify_initial_cloud_state() {
  ...
  local service_file service_url budget_mode unauthenticated_status response_file
  ...
  ! gcloud run services get-iam-policy "$SERVICE_NAME" --project "$PROJECT_ID" --region "$REGION" --format=json | \
    jq -e '[.bindings[]? | select(.role == "roles/run.invoker") | .members[]?] | index("allUsers") != null' >/dev/null \
    || die "Cloud Run must not grant allUsers before the canary"
  ...
  append_summary "Machine-read initial cloud: private, disabled, normal budget, App Check required, pinned public identifiers."
}
...
verify_device_preconditions() {
  ...
  append_summary "Machine-read device: paired, Developer Mode enabled, DDI usable, explicitly unlocked, uniquely resolved."
}
...
rolling_quota_digest() {
  local access_token documents_file response_file status page_token next_page_token query_url encoded count digest page_count
  access_token="$(gcloud auth print-access-token)" || return 1
  documents_file="$(mktemp "$TEMP_ROOT/firestore-digest-documents.XXXXXX")" || return 1
  ...
  while true; do
    ...
    response_file="$(mktemp "$TEMP_ROOT/firestore-digest-response.XXXXXX")" || return 1
    status="$(printf 'header = "Authorization: Bearer %s"\n' "$access_token" | \
  ...
  unset access_token
  count="$(wc -l <"$documents_file" | tr -d ' ')"
  digest="$(LC_ALL=C sort "$documents_file" | shasum -a 256 | awk '{print $1}')"
  rm -f "$documents_file"
  jq -cn --arg digest "$digest" --argjson count "$count" '{digest: $digest, documentCount: $count}'
}
...
capture_phase_window() {
  ...
  local start_utc="$2"
  local end_utc="$3"
  local quota_before="$4"
  local quota_after="$5"
  local output_file="$6"
  local raw_file normalized_file raw_count
  raw_file="$TEMP_ROOT/${phase}-raw-logs.json"
  normalized_file="$TEMP_ROOT/${phase}-safe-events.json"
  gcloud logging read \
    "resource.type=\"cloud_run_revision\" AND resource.labels.revision_name=\"$CANARY_REVISION\" AND timestamp>=\"$start_utc\" AND timestamp<=\"$end_utc\" AND (logName:\"run.googleapis.com%2Fstdout\" OR logName:\"run.googleapis.com%2Fstderr\")" \
    --project "$PROJECT_ID" \
    --limit=$((MAX_LOG_ENTRIES + 1)) \
    --format=json >"$raw_file"
  chmod 0600 "$raw_file"
  assert_no_prohibited_content "$raw_file" || die "Prohibited content was detected in canary logs; raw logs will not be retained"
  raw_count="$(jq 'length' "$raw_file")"
  (( raw_count <= MAX_LOG_ENTRIES )) || die "Canary log window exceeded the bounded $MAX_LOG_ENTRIES-entry limit"
  jq '
    [
      .[] |
      (
        if (.jsonPayload.event? == "meal_scan_scanner_event") then .jsonPayload
        elif (.textPayload? | type) == "string" then (try (.textPayload | fromjson) catch {})
        else {} end
      ) |
      select(.event == "meal_scan_scanner_event") |
      {
        eventType: (.eventType // ""),
        outcome: (.outcome // ""),
        control: (.control // ""),
  ...
  rm -f "$raw_file"
  jq -n \
    --slurpfile events "$normalized_file" \
    --arg quota_before "$(jq -r '.digest' <<<"$quota_before")" \
    --arg quota_after "$(jq -r '.digest' <<<"$quota_after")" '
      ($events[0]) as $e |
      {
        requestCompleted: ([$e[] | select(.eventType == "request_result" and .outcome == "completed")] | length),
        providerStarted: ([$e[] | select(.eventType == "provider_call" and .outcome == "started")] | length),
        providerCompleted: ([$e[] | select(.eventType == "provider_call" and .outcome == "completed")] | length),
        quotaAllowed: ([$e[] | select(.eventType == "quota_decision" and .outcome == "allowed")] | length),
        cacheFreshDispatch: ([$e[] | select(.eventType == "cache_decision" and .cacheDisposition == "fresh_dispatch")] | length),
        cacheServerHit: ([$e[] | select(.eventType == "cache_decision" and (.cacheDisposition | startswith("server_hit")))] | length),
        appCheckRejections: ([$e[] | select(.eventType == "authorization_rejection" and .control == "app_check")] | length),
        storeKitRejections: ([$e[] | select(.eventType == "authorization_rejection" and .control == "storekit_jws")] | length),
        revenueCatRejections: ([$e[] | select(.eventType == "authorization_rejection" and .control == "revenuecat_subscription")] | length),
        prohibitedContentDetected: false,
        rollingQuotaDigestBefore: $quota_before,
        rollingQuotaDigestAfter: $quota_after
      }
    ' >"$output_file"
...
capture_and_verify_phase() {
  ...
  local quota_before quota_after start_utc end_utc evidence_file
  quota_before="$(rolling_quota_digest)" || die "Unable to read content-free rolling quota digest before $phase"
  ...
  quota_after="$(rolling_quota_digest)" || die "Unable to read content-free rolling quota digest after $phase"
  sleep 10
  evidence_file="$TEMP_ROOT/${phase}-evidence.json"
  capture_phase_window "$phase" "$start_utc" "$end_utc" "$quota_before" "$quota_after" "$evidence_file"
  ...
  case "$phase" in
    exact-reuse)
      ...
      append_summary "Machine-read exact reuse: zero request, quota, cache, and provider activity; rolling quota digest unchanged."
      ;;
    fresh-scan|scan-as-new)
      append_summary "Owner-observed UI evidence: $phase flow was owner-reported; not machine proof."
      append_summary "Machine-read $phase: one completed request, App Check/Apple/RevenueCat path accepted, one quota decision, one fresh dispatch, one completed provider operation, no prohibited log content."
      ;;
  esac
}

restore_original_app_absence() {
  [[ "$APP_WAS_INSTALLED" == false && "$CANARY_APP_INSTALL_STARTED" == true ]] || return 0
...
rollback() {
  ...
  if [[ "$ROLLBACK_ARMED" == true && "$ROLLBACK_COMPLETE" == false ]]; then
    note "Rollback: redeploying disabled/private and verifying transport plus feature kill switch."
    if deploy_disabled_private && verify_final_disabled_private; then
      ROLLBACK_COMPLETE=true
      append_summary "Machine-read rollback: final service private; anonymous 403/404; authenticated 503 feature_disabled."
  ...
  if ! restore_original_app_absence; then
    note "FATAL: transient canary app could not be removed from a device where it was initially absent." >&2
    status=1
  fi
  APPLE_IAP_KEY_ID_VALUE=""
  APPLE_IAP_ISSUER_ID_VALUE=""
...
main() {
  ...
  for command in gcloud jq curl node npm xcodebuild xcrun codesign security plutil shasum git awk sed grep find; do
