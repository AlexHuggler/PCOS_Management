...
- Current-RC local validation passed 70 focused iOS tests, a dual-architecture Release simulator build (`arm64` and `x86_64`), and an unsigned generic-device archive. The inspected app reports `1.0.5 (18)`, bundle `alex.PCOS`, both signed scanner gates false, a valid privacy manifest, and a dSYM. Its bundle scan found only the expected public Firebase and RevenueCat client keys, with no Gemini/server/private-key material. This does not replace a signed distribution archive or IPA validation.
...
- Build `1.0.4` (`17`) was archived, exported, uploaded, reviewed, and released. The exported IPA is at `Artifacts/Exports/CycleBalance-1.0.4-b17/PCOS.ipa`.
