...
- The production meal-scan proxy, Firebase App Check/App Attest validation, Apple StoreKit transaction/status verification, HMAC-derived purchase-principal controls, Firestore rolling quotas/cache, and budget controller are deployed in `cyclebalance-prod-20260710` but remain private and disabled. RevenueCat remains the app's paywall/purchase layer, not proxy authorization evidence.
...
- Before an enabled review build, add explicit per-upload permission that names Google Gemini. Also verify production-project Zero Data Retention or disclose Google's standard 55-day abuse-monitoring retention everywhere the feature's data handling is described.
...
- Before release, verify the exact candidate archive against the production RevenueCat offering, monthly/annual packages, localized products, pricing, purchase, restore, and receipt synchronization. This remains incomplete and is separate from server-side StoreKit authorization.
...
- Fresh proxy access uses a verified StoreKit 2 transaction JWS and live Apple subscription status. The backend derives an HMAC principal from the verified original transaction identifier; no RevenueCat App User ID is sent as authorization evidence.
