...
- The negative physical App Check probe tooling is staged but must be rerun with the current StoreKit-JWS contract after the App Attest distribution profile is regenerated. It does not replace the later positive sandbox-JWS TestFlight path. Before enabling Photo Estimate for App Review, pass the complete 120-call benchmark, update and verify App Privacy, complete native-speaker/legal review, confirm reviewer access, and capture final screenshots from the exact frozen archive.
